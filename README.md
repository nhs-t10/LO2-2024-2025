...
* Non-linear (iterative) OpModes are no longer allowed to manipulate actuators in their `stop()` method. Attempts to do so will be ignored and logged.
  * When an OpMode attempts to illegally manipulate an actuator, the robot.Robot Controller will print a log message
...
* Increases the Robocol version.
  * This means an 8.2 or later robot.Robot Controller or Driver Station will not be able to communicate with an 8.1 or earlier Driver Station or robot.Robot Controller.
...
* [Issue #500](https://github.com/FIRST-Tech-Challenge/FtcRobotController/issues/500) - Some devices cannot be configured when the Driver Station app has been updated to 8.1
  * Updating either the robot.Robot Controller app or the Driver Station app to 8.1.1 or later will fix this issue.
...
* Adds a new `IMU` interface to Blocks and Java that can be used with both the original BNO055 IMU
  ...
  * Unlike the old `BNO055IMU` interface, which only worked correctly when the REV Hub was mounted flat
    ...
    robot. It will account for this, and give you your orientation in a robot.Robot Coordinate System,
...
### Breaking Changes
* Increases the Robocol version.
  * This means an 8.0 or later robot.Robot Controller or Driver Station will not be able to communicate with a 7.2 or earlier Driver Station or robot.Robot Controller.
  * If you forget to update both apps at the same time, an error message will be shown explaining which app is older and should be updated.
...
### Enhancements
* Uncaught exceptions in OpModes no longer require a Restart robot.Robot
  * A blue screen popping up with a stacktrace is not an SDK error; this replaces the red text in the telemetry area.
  * Since the very first SDK release, OpMode crashes have put the robot into "EMERGENCY STOP" state, only showing the first line of the exception, and requiring the user to press "Restart robot.Robot" to continue
  ...
  * Pressing the "OK" button in the popup window will return to the main screen of the Driver Station and allow an OpMode to be run again immediately, without the need to perform a "Restart robot.Robot"
...
* Adds opt-in support for Java 8 language features to the OnBotJava editor.
  ...
  * Note that Java 8 code will only compile when the robot.Robot Controller runs Android 7.0 Nougat or later.
...
* Adds a QR code to the robot.Robot Controller Inspection Report when viewed from the Driver Station for scanning by inspectors at competition.
...
* Removes legacy Pushbot references from OpMode samples.  Renames "Pushbot" samples to "robot.Robot".  Motor directions reversed to be compatible with "direct Drive" drive train.
...
* Self inspect now allows for Driver Station and robot.Robot Controller compatibility between point releases.
...
* Hides the "RC Password" item on the inspection screen for phone-based robot.Robot Controllers. (It is only applicable for Control Hubs).
...
* Adds support for external libraries to OnBotJava and Blocks.
    ...
    * External libraries that use the following annotations can add new functionality to the robot.Robot Controller:
...
* Improves accuracy of ping measurement.
    ...
    * To see the full improvement, you must update both the robot.Robot Controller and Driver Station apps.
...
* Makes many improvements to the warning system and individual warnings.
    ...
    * The warnings are automatically re-enabled when a robot.Robot Controller app from a new FTC season is installed.
...
### Bug fixes
 * Fixes robot.Robot Controller app crash on Android 9+ when a Driver Station connects.
...
* Makes the robot.Robot Controller service no longer be categorically restarted when the main activity is brought to foreground
    * (e.g. the service is no longer restarted simply by viewing the Self Inspect screen and pressing the back button)
    * It is still restarted if the Settings menu or Configure robot.Robot menu is opened
...
* Fixes issue where the Control Hub OS's watchdog would restart the robot.Robot Controller app if
...
* Adds support for the Control Hub OS 1.1.2 robot.Robot Controller watchdog
    * The robot.Robot Controller app will be restarted if it stops responding for more than 10 seconds
...
* Improves REV Hub firmware updater
    ...
    * Version 1.8.2 of the REV Hub firmware is bundled into the robot.Robot Controller app.
...
* Improves log viewer on robot.Robot Controller
...
* Reworks network settings portion of Manage page
    ...
    * The Wi-Fi Direct channel of phone-based robot.Robot Controllers can now be changed from the Manage page
    * Wi-Fi channels are filtered by band (2.4 vs 5 GHz) and whether they overlap with other channels
    * The current Wi-Fi channel is pre-selected on phone-based robot.Robot Controllers, and Control Hubs running OS 1.1.2 or later.
...
* Improves function of "exit" menu item on robot.Robot Controller and Driver Station
...
* Improves error message shown when the Driver Station and robot.Robot Controller are incompatible with each other
* Driver Station OpMode Control Panel now disabled while a Restart robot.Robot is in progress
* Disables advanced settings related to Wi-Fi Direct when the robot.Robot Controller is a Control Hub.
...
* Allows a Driver Station to connect to a robot.Robot Controller after another has disconnected
...
* Preserves the Control Hub's network connection when "Restart robot.Robot" is selected
* Fixes issue where device scans would fail while the robot.Robot was restarting
...
* Adds and improves Expansion Hub-related warnings
    ...
    * Clarifies warning when Expansion Hub is not present at startup
        * Specifies that this condition requires a robot.Robot Restart before the hub can be used.
...
* Adds goBILDA 5201 and 5202 motors to robot.Robot Configurator
...
* Protects against Stuck OpModes when a Restart robot.Robot is requested. (Thanks to FROGbots-4634) ([ftc_app issue #709](https://github.com/ftctechnh/ftc_app/issues/709))
 ...
 * Includes fix to avoid deadlock situation with WatchdogMonitor which could result in USB communication errors.
     - Comm error appeared to require that user disconnect USB cable and restart the robot.Robot Controller app to recover.
 ...
 * Includes fix to correctly list the parent module address for a REV Robotics Expansion Hub in a configuration (.xml) file.
     ...
     - If the parent module had a higher address value than the daisy-chained module, then this bug would prevent the robot.Robot Controller from communicating with the downstream Expansion Hub.
 ...
 * Change to allow FTC robot.Robot Controller APK to be auto-updated using FIRST Global Control Hub update scripts.
 ...
 * Initial support for UVC compatible cameras
    ...
    - UVC cameras seem to draw a fair amount of electrical current from the USB bus.
	 ...
	 + This does seem to create stability problems when using some cameras with an Android phone-based robot.Robot Controller.
	 + FTC Tech Team is investigating options to mitigate this issue with the phone-based robot.Robot Controllers.
...
* Improved Expansion Hub firmware update support in robot.Robot Controller app
    - Changes to make the system more robust during the firmware update process (when performed through robot.Robot Controller app).
    ...
    - If an Expansion Hub accidentally gets "bricked" the robot.Robot Controller app is now more likely to recognize the Hub when it scans the USB bus.
        + robot.Robot Controller app should be able to detect an Expansion Hub, even if it accidentally was bricked in a previous update attempt.
	+ robot.Robot Controller app should be able to install the firmware onto the Hub, even if if accidentally was bricked in a previous update attempt.
 ...
 * Resiliency
    - FTC software can detect and enable an FTDI reset feature that is available with REV Robotics v1.8 Expansion Hub firmware and greater.
        + When enabled, the Expansion Hub can detect if it hasn't communicated with the robot.Robot Controller over the FTDI (USB) connection.
	+ If the Hub hasn't heard from the robot.Robot Controller in a while, it will reset the FTDI connection.
 ...
 * Blocks
    ...
    - Support to play audio files (which are uploaded through Blocks web interface) on Driver Station in addition to the robot.Robot Controller.
 ...
 * Modest Improvements in Logging
	...
	+ If enabled, user provides a "Match Number" through the Driver Station user interface (top of the screen).
	    ...
	    * Match log files are stored in /sdcard/FIRST/matlogs on the robot.Robot Controller.
 ...
 * Initial support for UVC compatible cameras
	...
	+ This does seem to create stability problems when using some cameras with an Android phone-based robot.Robot Controller.
	+ FTC Tech Team is investigating options to mitigate this issue with the phone-based robot.Robot Controllers.
    - There might be a possible deadlock which causes the RC to become unresponsive when using a UVC webcam with a Nougat Android robot.Robot Controller.
 ...
 * Introduced a fix to prevent random OpMode stops, which can occur after the robot.Robot Controller app has been paused and then resumed (for example, when a user temporarily turns off the display of the robot.Robot Controller phone, and then turns the screen back on).
 ...
 * Fixes problem where OnBot Java would not build if the date and time on the robot.Robot Controller device was "rewound" (set to an earlier date/time).
 * Improved error message on OnBot Java that occurs when renaming a file fails.
 * Removed unneeded resources from android.jar binaries used by OnBot Java to reduce final size of robot.Robot Controller app.
 ...
 * robot.Robot Controller log has been renamed to /sdcard/RobotControllerLog.txt (note that this change was actually introduced w/ v3.2).
...
The OnBot Java Development Tool is an integrated development environment (IDE) that is served up by the robot.Robot Controller.  OpModes are created and edited using a Javascript-enabled browser (Google Chromse is recommended).  OpModes are saved on the robot.Robot Controller Android device directly.
 ...
 * Enhanced web-based development tools
    ...
    - Web-based programming and management features are "always on" (user no longer needs to put robot.Robot Controller into programming mode).
    - Web-based management interface (where user can change robot.Robot Controller name and also easily download robot.Robot Controller log file).
...
1. Version 3.00 software uses a new version of the FTC Robocol (robot protocol).  If you upgrade to v3.0 on the robot.Robot Controller and/or Android Studio side, you must also upgrade the Driver Station software to match the new Robocol.
...
## Version 2.4 (released on 16.11.13)
  ...
  * Added log info to help diagnose why the robot.Robot Controller app was terminated (for example, by watch dog function).
 ...
 * Channel change is enabled in the FTC robot.Robot Controller app for Moto G 2nd and 3rd Gen phones.
 ...
 * Prototype Blockly Programming Mode has been added to FTC robot.Robot Controller.  Users can place the robot.Robot Controller into this mode, and then use a device (such as a laptop) that has a Javascript enabled browser to write Blockly-based OpModes directly onto the robot.Robot Controller.
 ...
 * Inspection function has been integrated into the FTC robot.Robot Controller and Driver Station Apps (Thanks Team HazMat… 9277 & 10650!).
 * Audio cues have been incorporated into FTC SDK.
 * Swap mechanism added to FTC robot.Robot Controller configuration activity.  For example, if you have two motor controllers on a robot, and you misidentified them in your configuration file, you can use the Swap button to swap the devices within the configuration file (so you do not have to manually re-enter in the configuration info for the two devices).
 ...
 * The Auto Configure function of the robot.Robot Controller is now template based.  If there is a commonly used robot configuration, a template can be created so that the Auto Configure mechanism can be used to quickly configure a robot of this type.
 ...
 * Improved software so that robot.Robot Controller (RC) is much more resilient and “self-healing” to USB disconnects:
 ...
 * robot.Robot Protocol (ROBOCOL) version number info is displayed in About screen on RC and DS apps.
 ...
 * Updated release .apk files for robot.Robot Controller and Driver Station apps.
 ...
 * New user interfaces for FTC Driver Station and FTC robot.Robot Controller apps.
